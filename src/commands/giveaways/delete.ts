        ...
        try {
            if (/^\d+$/.test(inputId) && inputId.length < 10) {
                ...
                try {
                    ...
                    if (editFn) {
                        const reply = await editFn({ content: msg, flags: [MessageFlags.Ephemeral] });
                        setTimeout(async () => {
                            try {
            ...
            const editFn = ctx.editReply || ctx.reply;
            if (editFn) {
                await editFn({ content: msg, flags: [MessageFlags.Ephemeral] });
                setTimeout(async () => {
                    try {
                        if (ctx.deleteReply) await ctx.deleteReply().catch(() => {});
                    } catch (e) { }
            ...
            const editFn = ctx.editReply || ctx.reply;
            if (editFn) await editFn({ content: msg, flags: [MessageFlags.Ephemeral] });
        }
    }
};
