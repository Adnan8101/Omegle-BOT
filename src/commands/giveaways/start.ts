...
import { createGiveawayEmbed } from '../../util/giveaway/embeds';
...
import { validateDuration, calculateEndTimeFromString, toBigInt } from '../../util/giveaway/timeUtils';
const GIVEAWAY_EMOJI = '<a:Giveaway:1469356628565557413>';
export default {
    ...
    async run(ctx: any, channel: TextChannel, durationStr: string, winners: number, prize: string) {
        ...
        if (!validation.isValid) {
            ...
            return editFn?.({ content: `${Emojis.CROSS} ${validation.error}`, flags: [MessageFlags.Ephemeral] });
        }
        const endTimeMs = calculateEndTimeFromString(durationStr);
        if (!endTimeMs) {
            const editFn = ctx.editReply || ctx.reply;
            return editFn?.({ content: `${Emojis.CROSS} Invalid duration. Use: 30s, 2m, 1h, 7d`, flags: [MessageFlags.Ephemeral] });
        }
