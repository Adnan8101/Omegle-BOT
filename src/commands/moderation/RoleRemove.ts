...
export const RoleRemove: Command = {
    ...
    category: 'Moderation',
    syntax: 'roleremove <member> <role>',
    example: 'roleremove @User Moderator',
    ...
    execute: async (ctx: Context, args: string[]) => {
        ...
        if (args.length < 2) {
            const embed = new EmbedBuilder()
                .setDescription(`${CROSS} **Usage:** \`!roleremove <member> <role>\`\n**Example:** \`!roleremove @User Moderator\``);
            await ctx.reply({ embeds: [embed] });
            return;
        }
        ...
        if (targetRole && isExactRoleMatch(roleInput, targetRole)) {
            return await removeRoleFromMember(ctx, targetMember, targetRole);
        }
        ...
        if (!targetRole) {
            ...
            if (fuzzyMatch.similarity < 0.95) {
                const embed = new EmbedBuilder()
                    .setDescription(
                        `⚠️ **Role Confirmation**\n\n` +
                        `Did you mean to remove <@&${targetRole.id}> from ${targetMember.user}?\n\n` +
                        `**You typed:** \`${roleInput}\`\n` +
                        `**Best match:** <@&${targetRole.id}>\n\n` +
                        `*Use the exact role name, mention, or ID to remove the role.*`
                    );
        ...
        await removeRoleFromMember(ctx, targetMember, targetRole);
    }
};
