...
import { Client, MessageReaction, PartialMessageReaction, User, PartialUser, TextChannel, EmbedBuilder, AttachmentBuilder } from 'discord.js';
import { checkAllRequirements } from '../util/giveaway/requirements';
import { Theme } from '../util/giveaway/theme';
import { Emojis } from '../util/giveaway/emojis';
import { giveawayUpdateManager } from '../util/giveaway/GiveawayUpdateManager';
import { 
    getCachedGiveaway, 
    isParticipantCached, 
...
} from '../util/giveaway/giveawayCache';
...
export async function handleMessageReactionAdd(reaction: MessageReaction | PartialMessageReaction, user: User | PartialUser, client: Client) {
    return handleGiveawayReactionAdd(reaction, user, client);
}

async function handleGiveawayReactionAdd(reaction: MessageReaction | PartialMessageReaction, user: User | PartialUser, client: Client) {
    if (user.bot) return;
    ...
    if (giveaway.captchaRequirement) {
        try {
            const { generateCaptcha } = await import('../util/giveaway/captcha');
...
export async function handleMessageReactionRemove(reaction: MessageReaction | PartialMessageReaction, user: User | PartialUser, client: Client) {
    return handleGiveawayReactionRemove(reaction, user, client);
}

async function handleGiveawayReactionRemove(reaction: MessageReaction | PartialMessageReaction, user: User | PartialUser, client: Client) {
    if (user.bot) return;
    if (reaction.partial) await reaction.fetch();
    if (user.partial) {
        try { await user.fetch(); } catch { return; }
