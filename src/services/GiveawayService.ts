...
import { Client, TextChannel, ActionRowBuilder, ButtonBuilder, ButtonStyle, Role, GuildMember, EmbedBuilder, Guild } from 'discord.js';
import { giveawayEndedEmbed, giveawayCancelledEmbed, createGiveawayEmbed } from '../util/giveaway/embeds';
import { Giveaway } from '@prisma/client'; // Assuming Giveaway is a Prisma model type
...
export class GiveawayService {
    ...
    private async canAssignRole(guild: Guild, roleId: string): Promise<{ canAssign: boolean; reason?: string }> {
        try {
    ...
    async startGiveaway(giveawayData: any): Promise<void> {
        ...
        const giveawayEmoji = giveawayData.emoji || "<a:Giveaway:1469356628565557413>";
        const message = await channel.send({ content: `${giveawayEmoji} **New Giveaway** ${giveawayEmoji}`, embeds: [embed] });
        await message.react(giveawayData.emoji || "<a:Giveaway:1469356628565557413>");
        ...
        if (channel && guild) {
            try {
                ...
                if (winners.length > 0) {
                    ...
                    let hostName = giveaway.hostId;
                    try {
                        const hostUser = await this.client.users.fetch(giveaway.hostId);
    ...
    private async refreshGiveawayEmbed(giveaway: Giveaway, winners: string[], channel: TextChannel): Promise<void> {
        try {
            ...
            if (giveawayMessage) {
                const gwEmoji = giveaway.emoji || '<a:Giveaway:1469356628565557413>';
            ...
            console.error(`❌ Failed to update giveaway message ${giveaway.messageId}:`, msg);
        }
    }
}
